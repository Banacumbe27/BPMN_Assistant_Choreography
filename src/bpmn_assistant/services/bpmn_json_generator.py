...
class BpmnJsonGenerator:
    ...
    def _find_collaboration_element(self, root: ET.Element):
        for elem in root.iter():
            if elem.tag.endswith("collaboration"):
                return elem
        return None

    def create_bpmn_json(self, bpmn_xml: str) -> Any:
        """
        Create the JSON representation of the process from the BPMN XML.

        If the XML contains a <collaboration>, a collaboration dict
        {participants, message_flows} is returned; otherwise the legacy bare
        process list is returned.

        Constraints (per process):
        ...
        collaboration = self._find_collaboration_element(root)
        if collaboration is not None:
            return self._build_collaboration(root, collaboration)

        process_element = self._find_process_element(root)
        return self._build_single_process(process_element)

    def _build_single_process(
        self, process_element: ET.Element
    ) -> list[dict[str, Any]]:
        """Parse one <process> element into the nested JSON process list."""
        # Reset per-process state so multiple pools don't bleed into each other.
        self.elements = {}
        self.flows = {}
        self.process = []
    ...
    def _build_collaboration(
        self, root: ET.Element, collaboration: ET.Element
    ) -> dict[str, Any]:
        """Parse a <collaboration> (pools + message flows) into a collaboration dict."""
        # Map processRef -> <process> element
        processes: dict[str, ET.Element] = {}
        for elem in root.iter():
            if elem.tag.endswith("process") and elem.get("id"):
                processes[elem.get("id")] = elem
        ...
        for child in collaboration:
            ...
            if tag == "participant":
                ...
                process_ref = child.get("processRef")
                if process_ref and process_ref in processes:
                    participant["process"] = self._build_single_process(
                ...
                if child.get("name"):
                    mf["label"] = child.get("name")
                message_flows.append(mf)

        return {"participants": participants, "message_flows": message_flows}
