...
In addition to single-pool processes, the assistant can generate:

* **Collaborations** - multiple participants (pools) each owning a process, communicating via message flows.
* **Choreographies** - the message-exchange protocol between participants, modeled with choreography tasks.

The `bpmn-auto-layout` library cannot lay out these diagrams, so the backend embeds diagram-interchange (DI)
coordinates directly (see `bpmn_layout_generator.py`); single processes are still laid out by the auto-layout
service as before. The model self-selects the diagram type from the description; an optional
`determine_diagram_type` classifier is available for disambiguation.
...
* Lanes (sub-partitions within a single pool) are not supported.
* The deterministic DI layout for collaborations/choreographies is functional but simpler than the
  `bpmn-auto-layout` engine used for single processes.
